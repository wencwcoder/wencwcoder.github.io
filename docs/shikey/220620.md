...
- async-await 语法：是同步语法，彻底消除回调函数。【1】执行 async 函数，返回的是一个 Promise 对象。【2】await 相当于 Promise then。【3】try...catch 可捕获异常，代替 Promise 的 catch。【4】await 后面的代码，即下一排的代码开始，都可以看作是 callback 里面的内容，即异步，会启动 event loop。
