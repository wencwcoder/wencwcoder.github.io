...
- 性能优化：原则是，多使用内存、缓存；减少 CPU 计算量；减少网络加载耗时。【1】加载更快：减小资源体积 -- 压缩代码；减少访问次数 -- 合并代码；缓存；SSR；CDN。【2】渲染更快：css 放在 head，js 放在 body 最下面；尽早执行 js；懒加载；减少对 DOM 的频繁操作。
...
- 安全：**XSS 跨站请求攻击** -- 通过嵌入 `<script>` 脚本，使用**替换特殊字符**来预防；**XSRF 跨站请求伪造** -- 利用身份验证的漏洞，使用 **post 接口** / **增加验证**来预防。
